...
function initListeners() {
    ...
    for(let i = 0; i < arrOfAddHealthBtns.length; i++) {
        arrOfAddHealthBtns[i].addEventListener("click", () => {
            let request = new XMLHttpRequest();
            request.onload = function() { updateCharactersList() };
            request.open("GET", `index.php?action=adjustHealth&healthNumber=${arrOfAddHealthBtns[i].previousElementSibling.value}&characterId=${characterIdInput.value = arrOfAddHealthBtns[i].parentNode.parentNode.getAttribute("data-characterId")}`)
            request.send();
        });
    }
    ...
    for(let i = 0; i < arrOfSubHealthBtns.length; i++) {
        arrOfSubHealthBtns[i].addEventListener("click", () => {
            let request = new XMLHttpRequest();
            request.onload = function() { updateCharactersList() };
            request.open("GET", `index.php?action=adjustHealth&healthNumber=${arrOfSubHealthBtns[i].nextElementSibling.value * -1}&characterId=${characterIdInput.value = arrOfAddHealthBtns[i].parentNode.parentNode.getAttribute("data-characterId")}`)
            request.send();
        });
    }
    ...
    deleteEnemiesBtn.addEventListener("click", () => {
        ...
        if(confirm('Are you sure you want delete all enemies ?')) {
            request.send("GET", `index.php?action=deleteAllEnemies`, true)
            request.open()
        }
    });
