        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            if ($row['enemy_id'] != null) {
                ...
                foreach ($spells as $spell) {
                    ...
                    if (!$spellResults) {
                        $curl = curl_init();
                        $url = "https://api.open5e.com/v2/spells/?document__key__in=srd-2024&search=" . urlencode(ucwords($spell));
                        curl_setopt_array($curl, [
                            CURLOPT_URL => $url,
                            CURLOPT_RETURNTRANSFER => true,
                            CURLOPT_TIMEOUT => 10
                        ]);
                        // echo "https://api.open5e.com/v2/spells/?document__key__in=srd-2024&name=".str_replace(' ', '%20', ucwords($spell))."<br>";
                ...
                foreach ($handles as $info) {
                    ...
                    if (isset($decodedData->results[0])) {
                        $spellData = $decodedData->results[0];

                        // Escape and map API fields to your DB schema
                        $name = $conn->real_escape_string(normalize_string($spellData->name ?? $spell));
                        $desc = $conn->real_escape_string(normalize_string($spellData->desc ?? ''));
                        $higher_level = $conn->real_escape_string(normalize_string($spellData->higher_level ?? ''));
                        $rangeNum = (int)($spellData->range ?? 0);
                        $material = $conn->real_escape_string(normalize_string($spellData->material_specified ?? ''));
                        $ritual = isset($spellData->ritual) ? (int)$spellData->ritual : 0;
                        $duration = $conn->real_escape_string(normalize_string($spellData->duration ?? ''));
                        $concentration = isset($spellData->concentration) ? (int)$spellData->concentration : 0;
                        $casting_time = $conn->real_escape_string(normalize_string($spellData->casting_time ?? ''));
                        $level_text = $conn->real_escape_string(normalize_string($spellData->level ?? ''));
                        $level = (int)($spellData->level ?? 0);
                        $school = $conn->real_escape_string(normalize_string($spellData->school->name ?? ''));
                        ...
                        echo "No API result for spell: $spell<br>";
                    }
